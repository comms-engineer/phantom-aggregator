...
def extract_full_article(url: str) -> dict[str, Any]:
    ...
    # requests defaults to ISO-8859-1 when the server omits a charset, which mangles
    # UTF-8 bytes (e.g. "€" becomes "Â¬"). Fall back to the sniffed encoding instead.
    ...
    text_content = _strip_boilerplate_tail(text_content)
    if not text_content:
...
class ArticleExtractorService:
    ...
    def _render_article_page(self, article: dict[str, Any]) -> str:
        lines = [
            f">{_escape_micron(article.get('title', 'Untitled Article'))}",
            "-",
        ]
        if article.get("author"):
            lines.append(f"Author: {_escape_micron(article['author'])}")
...
# Trafilatura occasionally leaves a trailing related-links/byline block on the page
# (e.g. BBC's "Related topics - Published5 days ago ..."); trim it off if present.
_BOILERPLATE_TAIL_RE = re.compile(r"\n*Related topics\b.*\Z", re.IGNORECASE | re.DOTALL)


def _strip_boilerplate_tail(text: str) -> str:
    return _BOILERPLATE_TAIL_RE.sub("", text).strip()


def _escape_micron(text: Any) -> str:
    """Escape backticks so untrusted article text can't hijack Micron formatting state."""
